...
const WALK_VELOCITY_THRESHOLD = 1.8   // m/s — brisk walk threshold to start event
const RESUME_VELOCITY_THRESHOLD = 2.2 // m/s — runner picks up again
...
function computeHRRecovery(
  ...
  let i = 0
  while (i < len) {
    if (hr[i] > ceiling && velocity[i] < WALK_VELOCITY_THRESHOLD) {
      const startHr = hr[i]
      ...
      for (let j = i + 1; j < len; j++) {
        ...
        if (hr[j] <= ceiling) { eventEnd = j; break }
        if (elapsed >= MAX_EVENT_SECONDS) { eventEnd = j; break }
        if (velocity[j] > RESUME_VELOCITY_THRESHOLD) { eventEnd = j; break }
        eventEnd = j
      }
